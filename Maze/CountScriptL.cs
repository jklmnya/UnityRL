...
namespace Maze {
    ...
    public class CountScriptL : MonoBehaviour {
        public Maze.AgentScriptNL[] NormalAgents;
        public Maze.AgentScriptBL[] BackAgents;
        ...
        private void Show() {
            ...
            foreach (Maze.AgentScriptNL Agent in NormalAgents) {
                int[] Data = Agent.getData();
                NormalData[0] += Data[0];
                NormalData[1] += Data[1];
            }
            foreach (Maze.AgentScriptBL Agent in BackAgents) {
                int[] Data = Agent.getData();
                BackData[0] += Data[0];
                BackData[1] += Data[1];
            }
